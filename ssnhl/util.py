...
class Preprocess:
    ...
    def _common_process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        _data["Left ear/Right ear"] = _data["Left ear/Right ear"].replace(["L", "R", "L/R"], [1, 2, 3])
        _data["days from onset to treatment (categorized)"] = _data["days from onset to treatment (categorized)"].replace(["≤7 days", "8-14 days", ">14days"], [0, 1, 2])
        _data["Curve type (affected side)"] = _data["Curve type (affected side)"].replace(["normal", "downsloping", "flat", "profound", "upsloping"], [0, 1, 2, 3, 4])
        _data["Curve type (contralateral)"] = _data["Curve type (contralateral)"].replace(["normal", "downsloping", "flat", "profound", "upsloping"], [0, 1, 2, 3, 4])
    ...
    def KNN(self, data: pd.DataFrame, result_col: str) -> pd.DataFrame:
        ...
        grid = GridSearchCV(
            ...
            {"n_neighbors": np.arange(1, int(np.sqrt(_filter_data.shape[0]))+1)},
            cv=5
        )
...
def train_regression_model(clf, X_train, y_train, X_test, y_test, n_class):
    ...
    if n_class == 2:
        ...
        # multi class
        y_test_one_hot = label_binarize(y_test, classes=np.arange(n_class))
        y_pred_one_hot = label_binarize(y_pred_proba, classes=np.arange(n_class))
        fpr, tpr, thersholds = roc_curve(y_test_one_hot.ravel(), y_pred_one_hot.ravel())
    roc_auc = auc(fpr, tpr)
    roc = [fpr, tpr, roc_auc]
